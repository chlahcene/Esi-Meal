...
public class Restaurant implements Serializable {
    ...
    public Restaurant(int nbChaise, int nbTable, Set<ClientFidele> clients, Set<Commande> cmdEnAttente, Set<Commande> cmdEffectue, Menu menu) {
        this.nbChaise = nbChaise;
        this.nbTable = nbTable;
        this.clients = clients;
        this.cmdEnAttente = cmdEnAttente;
        this.cmdEffectue = cmdEffectue;
        this.menu = menu;
    }
    ...
    public int nbCmdEffectue(Date d1, Date d2) {
        ...
        if (d2.after(d1)) {
            for (Commande c : cmdEffectue
                ...
                if (c.getHeureConsomation().after(d1) && c.getHeureConsomation().before(d2)) {
                    cpt++;
                }
            }
        }
    ...
    public Met metPlusCmd(Date d1, Date d2) {
        ...
        if (d2.after(d1)) {
            if (menu.sizeMenu()==0) return null ;
            else m2 = menu.getMet(0);
            if (cmdEffectue.size() == 0) {
                return null;
            } else {
                for (int i = 0; i < menu.sizeMenu(); i++) {
                    m1 = menu.getMet(i);
                    nbCmd1 = 0;
                    for (Commande c : cmdEffectue
                            ) {
        ...
        for (Commande c : cmdEffectue
            ...
            if (c.getHeureConsomation().after(d1) && c.getHeureConsomation().before(d2)) {
                ...
                for (Commande d : cmdEffectue
                    ...
                    if (d.getHeureConsomation().after(d1) && d.getHeureConsomation().before(d2)) {
    ...
    public Client clientPlusCmdSurPlace(Date d1, Date d2) {
        ...
        for (Commande c : cmdEffectue
            ...
            if (c.getHeureConsomation().after(d1) && c.getHeureConsomation().before(d2) && c instanceof CmdSurPlace) {
                cl1 = c.getClient();
                for (Commande d : cmdEffectue
                        ) {
                    if (d.getHeureConsomation().after(d1) && d.getHeureConsomation().before(d2) && d instanceof CmdSurPlace) {
                        if (cl1.equals(d.getClient())) nbCmd1++;
                    }
    ...
    public Client clientPlusCmdADomicil(Date d1, Date d2) {
        ...
        for (Commande c : cmdEffectue
            ...
            if (c.getHeureConsomation().after(d1) && c.getHeureConsomation().before(d2) && c instanceof CmdADomicile) {
                cl1 = c.getClient();
                for (Commande d : cmdEffectue
                        ) {
                    if (d.getHeureConsomation().after(d1) && d.getHeureConsomation().before(d2) && d instanceof CmdADomicile) {
                        if (cl1.equals(d.getClient())) nbCmd1++;
                    }
    ...
    public Client clientPlusCmdEvenement(Date d1, Date d2) {
        ...
        for (Commande c : cmdEffectue
            ...
            if (c.getHeureConsomation().after(d1) && c.getHeureConsomation().before(d2) && c instanceof CmdEvenement) {
                cl1 = c.getClient();
                for (Commande d : cmdEffectue
                        ) {
                    if (d.getHeureConsomation().after(d1) && d.getHeureConsomation().before(d2) && d instanceof CmdEvenement) {
                        if (cl1.equals(d.getClient())) nbCmd1++;
                    }
                }
                if (nbCmd1!=0) existeAuMoinCommende = true;
    ...
    public boolean supprimerRepas(String nomRepas){
        return this.menu.removeRepas(nomRepas);
    }
    public boolean supprimerBoisson(String nomBoisson){
        return this.menu.removeBoisson(nomBoisson);
    }
