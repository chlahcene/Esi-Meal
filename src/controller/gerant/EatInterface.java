...
public class EatInterface  implements  Initializable {
    ...
    public void setlistRepas() {
        for (int i = 0; i < Noyeau.res.getMenu().sizeMenu(); i++) {
        ...
        if (Noyeau.utilfid == false) // client normale
        {
            ...
            if (ClientInterface.vnm) // commande evenement
            {
                if (ClientInterface.birth) {
                    cmd = new CmdEvenement(Noyeau.res.nbCmdTotal()+1,ClientInterface.nomutilisateur, ClientInterface.prenomutilisateur, ClientInterface.numeroTelephone, menusoffic.get(1), n, TypeEvenement.ANNIVERSAIRE,ClientInterface.etudiant);
                }
    ...
    public void mainclique() {
        ...
        try {
            ...
            stage.setTitle(" controller.Main ");
            stage.show();
        } catch (IOException e) {
        }
    ...
    public void visualiser()
    {
        ...
        Parent root;
        try {
            root = FXMLLoader.load(getClass().getResource("/view/infointerface.fxml"));
            stage.setScene(new Scene(root));
            stage.setTitle("");
            stage.show();
        } catch (IOException e) {
